...
| `MS Project` | `MS Project/Index.html`, puis `assets/js/main.js` ; accès `full` conservé dans `services/gristService.js` | Catalogue `MsProjectNom.Nom` pour la liste ; lignes `MsProject` filtrées pour l'affichage ; `Planning_Projet` pour les correspondances | Remplacement/ajout/mise à jour de `MsProject` ; ajout dédupliqué dans `MsProjectNom` après import réussi ; mise à jour des lignes correspondantes de `Planning_Projet` | Aucun runtime Projet/Service. Au démarrage, seule la petite table `MsProjectNom` est lue. Tant qu'aucun nom n'est choisi, aucune lecture de `MsProject` n'est lancée. Après sélection, une requête REST autonome porte le filtre exact `MsProject.Nom = valeur sélectionnée`, puis le résultat est vérifié strictement côté client ; aucun fallback complet de `MsProject` n'est autorisé. |
...
| `MsProjectNom` | `fetchTable()` complet au démarrage | aucune colonne de contexte ; lecture de `Nom` uniquement par le widget | nettoyage, déduplication et tri côté widget |
| `MsProject` | REST filtré uniquement après sélection ou import explicite ; REST complet seulement dans les audits administratifs hors widget | `Nom` exact choisi dans `projectDropdown` | aucun fallback complet dans le widget ; aucun filtre `Service` ou `NomProjet` inventé |
