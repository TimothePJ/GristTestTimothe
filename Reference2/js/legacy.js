...
function docLabelFromRecord(record) {
  ...
  if (!listbox) return;
  const hideArchived = (document.getElementById('hideArchivedToggle')?.checked) || false;
  ...
  // Build unique (NomDocument, NumeroDocument) pairs for the selected project
  const map = new Map(); // NomDocument -> Set(NumeroDocument|null)
  (records || []).forEach(record => {
    if (!record) return;
    if (record.NomProjet !== selectedValue) return;
    if (hideArchived && record.Archive) return;

    const name = String(record.NomDocument || '').trim();
    if (!name) return;

    let num = record.NumeroDocument;
    if (num === '' || num === '-' || num === '_') num = null;

    if (!map.has(name)) map.set(name, new Set());
    map.get(name).add((() => { const n = (num == null ? null : Number(num)); return (!Number.isFinite(n) || n === 0) ? null : n; })());
  });
  ...
  for (const [name, setNums] of map.entries()) {
    ...
    nums.forEach(n => {
      ...
      const label = makeDocLabel(name, n); // now number-first
      opts.push({ value, label, name, n });
    });
  }

  // Global sort: numero asc (nulls last), then name
  opts.sort((a,b) => {
  ...
  listbox.appendChild(addMultipleOption);
    ...
    function populateTable() {
      ...
      const filteredRecords = records.filter((record) => {
        const sameProject = record.NomProjet === selectedProject;
        const sameDocName = record.NomDocument === selectedTable;
        const numeroOk    = (selectedDocNumber == null) || (record.NumeroDocument == null) || (String(record.NumeroDocument) === String(selectedDocNumber));
        return sameProject && sameDocName && numeroOk && (!hideArchived || !record.Archive);
      });

      // (removed) No forced merge of Numero+Nom in table cells; we only display NomDocument.

      if (filteredRecords.length === 0) return;

      const headers = Object.keys(filteredRecords[0]).filter((key) => key !== 'NomProjet' && key !== 'NumeroDocument' && key !== 'NomDocument' && key !== 'id');
            ...
            td.addEventListener('click', async () => {
              ...
              try {
                await grist.docApi.applyUserActions([
                  ['UpdateRecord', 'References_2', record.id, { Bloquant: newValue }]
                ]);
    ...
    function showEditDialog(record) {
      ...
      document.getElementById('editDescription').value = record.DescriptionObservationss || '';
    ...
    document.getElementById('addRowDialog').addEventListener('submit', async (e) => {
      ...
      try {
          ...
          const projectIndex = projets.Projet.indexOf(selectedFirstValue);
          ...
          if (isDuplicate) {
              ...
              const newRow = {
                  ...
                  NumeroDocument: numeroOrZero(parseNumeroForStorage(parsedDoc.numero)),
                      ...
                      Indice: indice,
                      Recu: recu,
                      DescriptionObservationss: description,
                      DateLimite: datelimite,
                      Service: serviceValue,
                      Chemin: cheminFromAddFile
                  };
                  userActions.push(['AddRecord', 'References_2', null, newRow]);
              });
          } else {
              const newRow = {
                  NomProjet: projectId,
    ...
    document.getElementById('editRowDialog').addEventListener('submit', (e) => {
      ...
      if (selectedRecordId) {
        grist.docApi.applyUserActions([
          ['UpdateRecord', 'References_2', selectedRecordId, updatedRow]
        ])
          .then(() => {
    ...
    document.getElementById('archiveOption').addEventListener('click', () => {
      if (selectedRecordId) {
        if (confirm("Êtes-vous sûr de vouloir archiver cette ligne ?")) {
          grist.docApi.applyUserActions([
            ['UpdateRecord', 'References_2', selectedRecordId, { Archive: true }]
          ])
            .then(() => {
    ...
    document.getElementById('deleteOption').addEventListener('click', async () => {
      ...
      try {
          await grist.docApi.applyUserActions([
              ['RemoveRecord', 'References_2', selectedRecordId]
          ]);
    ...
    document.getElementById('secondColumnListbox').addEventListener('change', function () {
      ...
      const selectedValue = makeDocLabel(parsed.numero, parsed.name);
    ...
    document.getElementById('addDocumentDialog').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        const projets = await grist.docApi.fetchTable('Projet');
        const projectIndex = projets.Projet.indexOf(selectedProject);
        if (projectIndex === -1) throw new Error("Projet introuvable.");
        const projectId = projets.id[projectIndex];
        ...
        const actions = newRows.map(row => ['AddRecord', 'References_2', null, row]);
    ...
    function getCurrentSelections(isAddTableAction = false) {
      ...
      const raw    = tableDropdown.value.trim();
      const parsed = parseDocValue(raw);
      selectedDocNumber = parsed.numero;
      selectedDocName   = parsed.name;

      if (!selectedProject || !parsed.name) return null;
      return { selectedProject, selectedTable: parsed.name };
    }
    ...
    async function saveChanges() {
      ...
      const columnMap = ['Emetteur', 'Reference', 'Indice', 'Recu', 'DescriptionObservationss'];
      const updates = [];
    ...
    document.getElementById('addProjectDialog').addEventListener('submit', async (e) => {
      ...
      try {
        const result = await grist.docApi.applyUserActions([
          ['AddRecord', 'Projet', null, { 'NumeroProjet': projectNumber, 'Projet': projectName }]
        ]);
  ...
  async function updateEmetteurList(excludeCustom = false, targetDropdownId = "emetteurDropdown") {
    ...
    try {
        ...
        const referenceTable = await grist.docApi.fetchTable('References_2');
    ...
    try {
      ...
      documentsData.forEach(doc => {
        ...
        selectedEmitters.forEach(emetteur => {
          ...
          actions.push(['AddRecord', 'References_2', null, newRow]);
        });
      });
