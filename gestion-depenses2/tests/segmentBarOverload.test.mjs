...
// lourde.
//
// Le jumeau de ce fichier vit dans planning-synchro/tests/ : les deux widgets
// doivent afficher la meme teinte pour la meme charge. Le calcul, lui, est
// unique (utils/monthLoad.js, module vendorise verrouille par le test de parite).
//
// COMMENT ON TESTE : `buildVisibleSegmentBars` et `renderSegmentBars` restent
// module-prives dans chargeTimeline.js. On suit le motif de
// chargePlanMonthLoadBar.test.mjs : on extrait le TEXTE REEL des fonctions et on
...
const source = fs.readFileSync(
  new URL("../assets/js/ui/chargeTimeline.js", import.meta.url),
  "utf8"
);

function extractBlock(header) {
  const start = source.indexOf(header);
  assert.ok(start >= 0, `bloc introuvable dans chargeTimeline.js : ${header}`);
  assert.equal(
    source.indexOf(header, start + 1),
    -1,
    `en-tete ambigue dans chargeTimeline.js : ${header}`
  );
...
function visibleSlots() {
  ...
  return [
    { slotIndex: 0, leftPx: 0, widthPx: 100, startAt: bounds.startAt, endAt: bounds.endAt },
  ];
}
...
// Monte les vraies fonctions de chargeTimeline.js et rend la barre produite.
function renderBars({ workerName = "Alice", segments, allSegmentRows, absenceSet = null }) {
  const sandbox = {
    Math,
    Number,
    String,
    ...
    getMonthBounds,
    formatNumber,
    // Les taches Planning Projet ne sont pas le sujet : la vraie fonction serait
    // du bruit, et son absence ferait echouer le montage plutot que le test.
    countPlanningTasksOverlappingRange: () => 0,
  };
  const context = vm.createContext(sandbox);
  vm.runInContext(
    [...BLOCKS, "globalThis.__api = { buildVisibleSegmentBars, renderSegmentBars };"].join("\n\n"),
    context,
    { filename: "chargeTimeline.js (extrait)" }
  );

  const worker = {
    id: 1,
    name: workerName,
    absenceSet: absenceSet instanceof Set ? absenceSet : new Set(),
    segments: segments.map((seg) => ({
      id: seg.id,
      monthKey: MONTH,
      effectifDays: seg.effectif,
    })),
  };
...
test("un segment vire a l'ambre quand la personne deborde SUR UN AUTRE projet", () => {
  ...
  const { html } = renderBars({
    ...
    allSegmentRows: [segmentRow(1, "Alice", 15, "A"), segmentRow(2, "Alice", 12, "B")],
  });
...
test("un mois PILE plein ne vire pas", () => {
  const { html } = renderBars({
    ...
    allSegmentRows: [segmentRow(1, "Alice", 14, "A"), segmentRow(2, "Alice", 8, "B")],
  });
...
test("le rouge « incoherent » l'emporte sur l'ambre", () => {
  ...
  const { html } = renderBars({
    ...
    allSegmentRows: [segmentRow(1, "Alice", 25, "A")],
  });
...
test("sans surcharge, l'infobulle reste celle des plans Planning Projet", () => {
  const { html } = renderBars({
    segments: [{ id: 1, effectif: 5 }],
    allSegmentRows: [segmentRow(1, "Alice", 5, "A")],
  });
...
test("le nom du segment est apparie sans egard aux accents ni a la casse", () => {
  // Les lignes TimeSegment et les noms affiches ne s'ecrivent pas toujours
  // pareil. Sans normalisation la barre resterait neutre EN SILENCE, ce qui est
  // indiscernable d'une personne qui n'est pas en surcharge.
  const { html } = renderBars({
    workerName: "ALICE",
    segments: [{ id: 1, effectif: 15 }],
    allSegmentRows: [segmentRow(1, "Alice", 15, "A"), segmentRow(2, "Alice", 12, "B")],
