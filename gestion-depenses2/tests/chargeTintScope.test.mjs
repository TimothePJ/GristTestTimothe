...
test("les teintes de charge sont declarees sur :root, avec les bonnes valeurs", () => {
  ...
  Object.entries(expected).forEach(([name, hex]) => {
    ...
    // Assertion POSITIVE, et non « different de .segment-edit-load » : un
...
test("la fenetre de segment consomme toujours les trois teintes", () => {
  ...
  assert.match(CSS, /\.segment-edit-load-fill\s*\{[^}]*var\(--load-partial\)/);
  assert.match(CSS, /\.segment-edit-load\.is-balanced[^{]*\{[^}]*var\(--load-balanced\)/);
  assert.match(CSS, /\.segment-edit-load\.is-overload[^{]*\{[^}]*var\(--load-overload\)/);
});
